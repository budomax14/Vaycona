...
function GuideRow({ guide, pageName, unit, onUpdate, onDelete, t }) {
  ...
  return (
    <div className="grid grid-cols-[20px_56px_1fr_70px_auto] items-center gap-2 rounded-lg px-2 py-1.5 text-xs hover:bg-gray-50">
      <span
        ...
        aria-hidden="true"
      />
      <span className="text-gray-500">{guide.orientation === "vertical" ? t.vertical : t.horizontal}</span>
      <input
        aria-label={t.guideLabelAria}
        ...
        placeholder={pageName || t.globalPlaceholder}
        onChange={(event) => setLabelDraft(event.target.value)}
        onBlur={() => onUpdate(guide.id, { label: labelDraft })}
      />
      <div className="flex items-center gap-1">
        <input
          aria-label={t.positionAria(unitLabel(unit))}
          type="text"
          inputMode="decimal"
      ...
      <div className="flex items-center gap-0.5">
        <button
          ...
          title={guide.locked ? t.unlockGuide : t.lockGuide}
          aria-label={guide.locked ? t.unlockGuide : t.lockGuide}
        ...
        <button
          ...
          title={guide.hidden ? t.showGuide : t.hideGuide}
          aria-label={guide.hidden ? t.showGuide : t.hideGuide}
          aria-pressed={guide.hidden}
        >
          {guide.hidden ? <EyeOff size={12} /> : <Eye size={12} />}
        </button>
        <div className="flex items-center gap-0.5 rounded border border-gray-100 p-0.5" role="radiogroup" aria-label={t.guideColorAria}>
          {GUIDE_COLORS.map((c) => (
            <button
              key={c.key}
              ...
              aria-label={t.setColorAria(c.key)}
              role="radio"
...
export default function GuideManagerDialog({
  ...
  const t = DIALOG_STRINGS[language].guideManager;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" role="presentation">
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="guide-manager-title" className="flex max-h-[85vh] w-full max-w-lg flex-col rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4">
          <h2 id="guide-manager-title" className="text-base font-semibold text-gray-900">{t.title}</h2>
        ...
        <div className="border-b border-gray-100 px-5 py-3">
          <div className="flex flex-wrap items-end gap-2">
            ...
            <label className="flex flex-col gap-1 text-xs text-gray-500">
              {t.positionLabel(unitLabel(unit))}
              <input
                type="text"
                inputMode="decimal"
            ...
            <label className="flex flex-col gap-1 text-xs text-gray-500">
              ...
              <select className="rounded-lg border border-gray-200 px-2 py-1.5 text-sm" value={newScope} onChange={(event) => setNewScope(event.target.value)}>
                <option value="page">{t.thisPageOption}</option>
                <option value="global">{t.allPagesOption}</option>
              </select>
            </label>
        ...
        <div className="flex items-center justify-between px-5 py-2 text-xs">
          <div className="flex gap-1">
            ...
            <button className={`rounded-full px-2.5 py-1 font-medium ${scopeFilter === "all" ? "bg-amber-100 text-amber-700" : "text-gray-500 hover:bg-gray-100"}`} onClick={() => setScopeFilter("all")}>
              {t.allPagesFilter(guides.length)}
            </button>
          </div>
          <span className="text-gray-400">{t.guideCount(visibleGuides.length)}</span>
        </div>

        <div className="flex-1 overflow-y-auto px-3 pb-2">
          {visibleGuides.length === 0 ? (
            <p className="px-2 py-6 text-center text-xs text-gray-400">{t.emptyState}</p>
          ) : (
            visibleGuides.map((guide) => (
              <GuideRow key={guide.id} guide={guide} pageName={pageName(guide.pageId)} unit={unit} onUpdate={onUpdate} onDelete={onDelete} t={t} />
            ))
          )}
        </div>
        ...
        <div className="flex justify-end gap-2 border-t border-gray-200 px-5 py-3">
          ...
          <button
            ...
            onClick={() => {
              if (window.confirm(t.confirmClearAll(guides.length))) onClearAll();
            }}
          >
            {t.clearAll}
          </button>
