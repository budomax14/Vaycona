...
export default function RecoveryCenter({
  ...
  const t = DIALOG_STRINGS[language].recoveryCenter;
  const REASON_LABELS = {
        ...
        <div className="border-b border-gray-100 px-5 py-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium text-gray-700">
              {saveStatus === "saved" ? t.savedOk : t.unsavedChanges}
            </span>
          </div>
          {lastSavedAt && <div className="mt-0.5 pl-6 text-xs text-gray-400">{t.lastSaved(formatTimestamp(lastSavedAt, t))}</div>}
        </div>
                ...
                >
                  <div>
                    <div className="flex items-center gap-1.5 text-sm font-medium text-gray-800">
                      {REASON_LABELS[snapshot.reason] || t.snapshotFallback}
                      {snapshot.protected && (
                    ...
                    <div className="flex items-center gap-1 text-xs text-gray-400">
                      <Clock size={11} /> {t.snapshotMeta(formatTimestamp(snapshot.createdAt, t), snapshot.pageCount, snapshot.objectCount)}
                    </div>
                  </div>
